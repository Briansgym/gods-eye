...
| **UCDP Candidate GED** (Uppsala Conflict Data Program, Uppsala University) | Global armed-conflict events: state-based conflict, non-state conflict, one-sided violence against civilians (monthly candidate release, rolling lag; served via the local `/api/conflicts` proxy from the official keyless dataset download) | [CC BY 4.0](https://ucdp.uu.se/downloads/) — cite UCDP / Uppsala University | "Armed conflicts: UCDP Candidate GED — Uppsala Conflict Data Program, Uppsala University (CC BY 4.0)" |
