...
test('every framing is a real oblique standoff, not a nadir or an inside-out sphere', () => {
  ...
  assert.deepEqual(kinds.sort(), ['entity', 'fire', 'vessel']);
  ...
  // Generic double-click dive (dams/datacenters/cables/installations): a
  // local inspect view on the 1–8 km order required by the 2026-09-11 brief.
  assert.deepEqual({ ...WORLD_FOCUS_FRAMING.entity }, { radiusM: 300, rangeM: 3500, pitchDeg: -35 });
});
